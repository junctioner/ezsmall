...
public class CartViewAction {
	...
	public void add_goods_cart(HttpServletRequest request, HttpServletResponse response, String id, String count,
		...
		if (add) {// 买家当前所选规格的商品可以添加到购物车
			...
			// 解析商品规格
			List<GoodsSpecProperty> gds_specs = null;
			if (goods.getGoods_property() != null && !"".equals(goods.getGoods_property())) {
				gds_specs = new ArrayList<GoodsSpecProperty>();
				List<GoodPec> goodpecs = MapTools.getGoodPecByStr(goods.getGoods_property());
				for (GoodPec pec : goodpecs) {
					GoodsSpecProperty property = new GoodsSpecProperty();
					property.setId(CommUtil.null2Long(pec.getId()));
					gds_specs.add(property);
				}
			}
			((GoodsCart) obj).setSc(sc33);
			((GoodsCart) obj).setGsps(gds_specs);
			((GoodsCart) obj).setSpec_info(goods.getGoods_property());
		...
		try {
			if (SecurityUserHolder.getCurrentUser() != null) {
				...
				if (id.equals("")) {
					...
					msgMap.put("msg", "新增收货地址成功！");
				} else {
					this.addressService.update(address);
					msgMap.put("code", "success");
					msgMap.put("msg", "修改收货地址成功！");
				}
			} else {
			...
			e.printStackTrace();
			msgMap.put("code", "failure");
			msgMap.put("msg", "系统通讯异常，请联系网站维护人员！");
		} finally {
			PrintWriter writer;
			try {
				writer = response.getWriter();
				writer.print(Json.toJson(msgMap, JsonFormat.compact()));
			} catch (IOException e) {
				e.printStackTrace();
			}
		}
