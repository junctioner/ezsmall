...
public class IndexViewAction {
	...
	public ModelAndView index(HttpServletRequest request, HttpServletResponse response) {
		...
		List<Goods> store_reommend_goods_list = this.goodsService.query("select obj from Goods obj where obj.goods_status=:goods_status and obj.goods_recommend =:goods_recommend order by obj.goods_click desc",params, 0, 4);
		mv.addObject("store_reommend_goods_list", store_reommend_goods_list);
		// 8、点击数最多:人气商品
		params.clear();
		params.put("goods_status", Integer.valueOf(0));
		List hot_goods_list = this.goodsService.query("select obj from Goods obj where obj.goods_status=:goods_status order by obj.goods_click desc", params,0, 24);
		...
		// List gbs = this.goodsBrandService.query("select obj from GoodsBrand
		// obj where obj.audit=:audit and obj.recommend=:recommend order by
		// obj.sequence", params, 0, 4);
		// mv.addObject("gbs", gbs);
		// // 底部显示的合作伙伴
		// params.clear();
		// List img_partners = this.partnerService.query("select obj from
		// Partner obj where obj.image.id is not null order by obj.sequence
		// asc", params, -1, -1);
		// mv.addObject("img_partners", img_partners);
		// List text_partners = this.partnerService.query("select obj from
		// Partner obj where obj.image.id is null order by obj.sequence asc",
		// params, -1, -1);
		// mv.addObject("text_partners", text_partners);
		// // 底部新闻分类显示
		// params.clear();
		// params.put("mark", "news");
		// List acs = this.articleClassService.query("select obj from
		// ArticleClass obj where obj.parent.id is null and obj.mark!=:mark
		// order by obj.sequence asc", params, 0, 9);
		// mv.addObject("acs", acs);
		// // 商城新闻
		// params.clear();
		// params.put("class_mark", "news");
		// params.put("display", Boolean.valueOf(true));
		// List articles = this.articleService.query("select obj from Article
		// obj where obj.articleClass.mark=:class_mark and obj.display=:display
		// order by obj.addTime desc", params, 0, 5);
		// mv.addObject("articles", articles);
		// // 查询推荐店铺商品
		// params.clear();
		// params.put("store_recommend", Boolean.valueOf(true));
		// params.put("goods_status", Integer.valueOf(0));
		// List store_reommend_goods_list = this.goodsService.query("select obj
		// from Goods obj where obj.store_recommend=:store_recommend and
		// obj.goods_status=:goods_status order by obj.store_recommend_time
		// desc", params, -1, -1);
		...
		// List groups = this.groupService.query("select obj from Group obj
		// where obj.beginTime<=:beginTime and obj.endTime>=:endTime", params,
		// -1, -1);
		// if(groups.size() > 0){
		// // 2、团购商品
		// params.clear();
		...
		// List ggs = this.groupGoodsService.query("select obj from GroupGoods
		// obj where obj.gg_status=:gg_status and obj.gg_recommend=:gg_recommend
		// and obj.group.id=:group_id order by obj.gg_recommend_time desc",
		// params, 0, 5);
		// //if(ggs.size() > 0)
		...
		// List bgs = this.bargainGoodsService.query("select obj from
		// BargainGoods obj where obj.bg_time=:bg_time and
		// obj.bg_status=:bg_status", params, 0, 5);
		// mv.addObject("bgs", bgs);
		// // 4、热销商品倒序-疯狂抢购
		// params.clear();
		// params.put("goods_status", Integer.valueOf(0));
		// List list = this.goodsService.query("select obj from Goods obj where
		// obj.goods_status=:goods_status order by obj.goods_salenum desc",
		// params, 0, 5);
		...
		// List dgs = this.deliveryGoodsService.query("select obj from
		// DeliveryGoods obj where obj.d_status=:d_status and
		// obj.d_begin_time<=:d_begin_time and obj.d_end_time>=:d_end_time order
		// by obj.d_audit_time desc", params, 0, 5);
		// mv.addObject("dgs", dgs);
		//
		// // 7、新品上架
		// params.clear();
		// params.put("goods_status", Integer.valueOf(0));
		// List new_goods_list = this.goodsService.query("select obj from Goods
		// obj where obj.goods_status=:goods_status order by obj.addTime desc",
		// params, 0, 5);
		...
		// params.clear();
		// params.put("goods_status", Integer.valueOf(0));
		// List hot_goods_list = this.goodsService.query("select obj from Goods
		// obj where obj.goods_status=:goods_status order by obj.goods_click
		// desc", params, 0, 5);
		// mv.addObject("hots", hot_goods_list);
	...
	public ModelAndView error404(HttpServletRequest request, HttpServletResponse response) {
		...
		if ((wemall_view_type != null) && (!wemall_view_type.equals("")) && (wemall_view_type.equals("wap"))) {
			// String store_id =
			// CommUtil.null2String(request.getSession(false).getAttribute("store_id"));
			...
			for (Goods goods : this.gf_tools.generic_goods(info.getGf_gc_goods())) {
				if (goods != null) {
					if (goods.getGoods_main_photo() != null)
						img = map.get("imageWebServer") + "/" + goods.getGoods_main_photo().getPath() + "/"
