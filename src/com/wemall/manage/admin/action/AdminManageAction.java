...
public class AdminManageAction implements ServletContextAware {
        ...
        for (String id : ids){
            if (!id.equals("")){
                ...
                if (!user.getUsername().equals("admin")){
                    this.databaseTools.execute("delete from wemall_syslog where user_id=" + id);
                    this.databaseTools.execute("delete from wemall_user_role where user_id=" + id);
    ...
    public String init_role(){
        ...
        if ((current_user != null) && (current_user.getUserRole().indexOf("ADMIN") >= 0) && (current_user.getUsername().equals("admin"))){
            this.databaseTools.execute("delete from wemall_role_res");
            this.databaseTools.execute("delete from wemall_res");
            this.databaseTools.execute("delete from wemall_user_role");
            this.databaseTools.execute("delete from wemall_role");
            this.databaseTools.execute("delete from wemall_rolegroup");
